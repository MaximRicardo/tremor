#pragma once

#include "ssize.hpp"
#include "vector/vec2.hpp"
#include <array>
#include <cstdint>

namespace Font {

constexpr int32_t char_width = 7;
constexpr int32_t char_height = 13;
inline Vec2i char_size = Vec2i(char_width, char_height);

constexpr isize_t n_chars = 95;

bool active_pixel(char c, Vec2i pos);

constexpr char bitmap_ascii_first = 32;
constexpr char bitmap_ascii_last = 126;
inline std::array<std::array<uint8_t, char_height>, n_chars> bitmaps = {
    {/*
      * code=32, hex=0x20, ascii=" "
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
     },
     /*
      * code=33, hex=0x21, ascii="!"
      */
     {
         0x00, /* 0000000 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x00, /* 0000000 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x00, /* 0000000 */
     },
     /*
      * code=34, hex=0x22, ascii="""
      */
     {
         0x00, /* 0000000 */
         0x6C, /* 0110110 */
         0x6C, /* 0110110 */
         0x44, /* 0100010 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
     },
     /*
      * code=35, hex=0x23, ascii="#"
      */
     {
         0x00, /* 0000000 */
         0x6C, /* 0110110 */
         0x6C, /* 0110110 */
         0xFE, /* 1111111 */
         0x6C, /* 0110110 */
         0xFE, /* 1111111 */
         0x6C, /* 0110110 */
         0x6C, /* 0110110 */
         0x00, /* 0000000 */
     },
     /*
      * code=36, hex=0x24, ascii="$"
      */
     {
         0x08, /* 0000100 */
         0x18, /* 0001100 */
         0x3C, /* 0011110 */
         0x60, /* 0110000 */
         0x3C, /* 0011110 */
         0x06, /* 0000011 */
         0x7C, /* 0111110 */
         0x18, /* 0001100 */
         0x10, /* 0001000 */
     },
     /*
      * code=37, hex=0x25, ascii="%"
      */
     {
         0x70, /* 0111000 */
         0x52, /* 0101001 */
         0x76, /* 0111011 */
         0x0C, /* 0000110 */
         0x18, /* 0001100 */
         0x3E, /* 0011111 */
         0x6A, /* 0110101 */
         0x0E, /* 0000111 */
         0x00, /* 0000000 */
     },
     /*
      * code=38, hex=0x26, ascii="&"
      */
     {
         0x38, /* 0011100 */
         0x6C, /* 0110110 */
         0x6C, /* 0110110 */
         0x38, /* 0011100 */
         0x6E, /* 0110111 */
         0x6C, /* 0110110 */
         0x6C, /* 0110110 */
         0x3E, /* 0011111 */
         0x00, /* 0000000 */
     },
     /*
      * code=39, hex=0x27, ascii="'"
      */
     {
         0x00, /* 0000000 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x30, /* 0011000 */
         0x30, /* 0011000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
     },
     /*
      * code=40, hex=0x28, ascii="("
      */
     {
         0x00, /* 0000000 */
         0x0C, /* 0000110 */
         0x18, /* 0001100 */
         0x30, /* 0011000 */
         0x30, /* 0011000 */
         0x30, /* 0011000 */
         0x18, /* 0001100 */
         0x0C, /* 0000110 */
         0x00, /* 0000000 */
     },
     /*
      * code=41, hex=0x29, ascii=")"
      */
     {
         0x00, /* 0000000 */
         0x30, /* 0011000 */
         0x18, /* 0001100 */
         0x0C, /* 0000110 */
         0x0C, /* 0000110 */
         0x0C, /* 0000110 */
         0x18, /* 0001100 */
         0x30, /* 0011000 */
         0x00, /* 0000000 */
     },
     /*
      * code=42, hex=0x2A, ascii="*"
      */
     {
         0x00, /* 0000000 */
         0x44, /* 0100010 */
         0x6C, /* 0110110 */
         0x38, /* 0011100 */
         0xFE, /* 1111111 */
         0x38, /* 0011100 */
         0x6C, /* 0110110 */
         0x44, /* 0100010 */
         0x00, /* 0000000 */
     },
     /*
      * code=43, hex=0x2B, ascii="+"
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x7E, /* 0111111 */
         0x7E, /* 0111111 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x00, /* 0000000 */
     },
     /*
      * code=44, hex=0x2C, ascii=","
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x30, /* 0011000 */
         0x30, /* 0011000 */
         0x60, /* 0110000 */
     },
     /*
      * code=45, hex=0x2D, ascii="-"
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x7C, /* 0111110 */
         0x7C, /* 0111110 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
     },
     /*
      * code=46, hex=0x2E, ascii="."
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x30, /* 0011000 */
         0x30, /* 0011000 */
         0x00, /* 0000000 */
     },
     /*
      * code=47, hex=0x2F, ascii="/"
      */
     {
         0x00, /* 0000000 */
         0x0C, /* 0000110 */
         0x0C, /* 0000110 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x30, /* 0011000 */
         0x30, /* 0011000 */
         0x60, /* 0110000 */
         0x00, /* 0000000 */
     },
     /*
      * code=48, hex=0x30, ascii="0"
      */
     {
         0x00, /* 0000000 */
         0x3C, /* 0011110 */
         0x66, /* 0110011 */
         0x6E, /* 0110111 */
         0x76, /* 0111011 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x3C, /* 0011110 */
         0x00, /* 0000000 */
     },
     /*
      * code=49, hex=0x31, ascii="1"
      */
     {
         0x00, /* 0000000 */
         0x18, /* 0001100 */
         0x78, /* 0111100 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x7E, /* 0111111 */
         0x00, /* 0000000 */
     },
     /*
      * code=50, hex=0x32, ascii="2"
      */
     {
         0x00, /* 0000000 */
         0x3C, /* 0011110 */
         0x66, /* 0110011 */
         0x46, /* 0100011 */
         0x1C, /* 0001110 */
         0x30, /* 0011000 */
         0x66, /* 0110011 */
         0x7E, /* 0111111 */
         0x00, /* 0000000 */
     },
     /*
      * code=51, hex=0x33, ascii="3"
      */
     {
         0x00, /* 0000000 */
         0x3C, /* 0011110 */
         0x66, /* 0110011 */
         0x06, /* 0000011 */
         0x1C, /* 0001110 */
         0x06, /* 0000011 */
         0x66, /* 0110011 */
         0x3C, /* 0011110 */
         0x00, /* 0000000 */
     },
     /*
      * code=52, hex=0x34, ascii="4"
      */
     {
         0x00, /* 0000000 */
         0x0C, /* 0000110 */
         0x1C, /* 0001110 */
         0x3C, /* 0011110 */
         0x6C, /* 0110110 */
         0x7E, /* 0111111 */
         0x0C, /* 0000110 */
         0x1E, /* 0001111 */
         0x00, /* 0000000 */
     },
     /*
      * code=53, hex=0x35, ascii="5"
      */
     {
         0x00, /* 0000000 */
         0x7E, /* 0111111 */
         0x66, /* 0110011 */
         0x60, /* 0110000 */
         0x7C, /* 0111110 */
         0x06, /* 0000011 */
         0x66, /* 0110011 */
         0x3C, /* 0011110 */
         0x00, /* 0000000 */
     },
     /*
      * code=54, hex=0x36, ascii="6"
      */
     {
         0x00, /* 0000000 */
         0x1C, /* 0001110 */
         0x30, /* 0011000 */
         0x60, /* 0110000 */
         0x7C, /* 0111110 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x3C, /* 0011110 */
         0x00, /* 0000000 */
     },
     /*
      * code=55, hex=0x37, ascii="7"
      */
     {
         0x00, /* 0000000 */
         0x7E, /* 0111111 */
         0x66, /* 0110011 */
         0x06, /* 0000011 */
         0x0C, /* 0000110 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x00, /* 0000000 */
     },
     /*
      * code=56, hex=0x38, ascii="8"
      */
     {
         0x00, /* 0000000 */
         0x3C, /* 0011110 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x3C, /* 0011110 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x3C, /* 0011110 */
         0x00, /* 0000000 */
     },
     /*
      * code=57, hex=0x39, ascii="9"
      */
     {
         0x00, /* 0000000 */
         0x3C, /* 0011110 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x3E, /* 0011111 */
         0x06, /* 0000011 */
         0x0C, /* 0000110 */
         0x38, /* 0011100 */
         0x00, /* 0000000 */
     },
     /*
      * code=58, hex=0x3A, ascii=":"
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x30, /* 0011000 */
         0x30, /* 0011000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x30, /* 0011000 */
         0x30, /* 0011000 */
         0x00, /* 0000000 */
     },
     /*
      * code=59, hex=0x3B, ascii=";"
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x30, /* 0011000 */
         0x30, /* 0011000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x30, /* 0011000 */
         0x30, /* 0011000 */
         0x60, /* 0110000 */
     },
     /*
      * code=60, hex=0x3C, ascii="<"
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x18, /* 0001100 */
         0x30, /* 0011000 */
         0x60, /* 0110000 */
         0x30, /* 0011000 */
         0x18, /* 0001100 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
     },
     /*
      * code=61, hex=0x3D, ascii="="
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x7C, /* 0111110 */
         0x00, /* 0000000 */
         0x7C, /* 0111110 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
     },
     /*
      * code=62, hex=0x3E, ascii=">"
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x30, /* 0011000 */
         0x18, /* 0001100 */
         0x0C, /* 0000110 */
         0x18, /* 0001100 */
         0x30, /* 0011000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
     },
     /*
      * code=63, hex=0x3F, ascii="?"
      */
     {
         0x00, /* 0000000 */
         0x3C, /* 0011110 */
         0x66, /* 0110011 */
         0x06, /* 0000011 */
         0x0C, /* 0000110 */
         0x18, /* 0001100 */
         0x00, /* 0000000 */
         0x18, /* 0001100 */
         0x00, /* 0000000 */
     },
     /*
      * code=64, hex=0x40, ascii="@"
      */
     {
         0x00, /* 0000000 */
         0x3C, /* 0011110 */
         0x62, /* 0110001 */
         0x6E, /* 0110111 */
         0x6A, /* 0110101 */
         0x6C, /* 0110110 */
         0x62, /* 0110001 */
         0x3C, /* 0011110 */
         0x00, /* 0000000 */
     },
     /*
      * code=65, hex=0x41, ascii="A"
      */
     {
         0x00, /* 0000000 */
         0x10, /* 0001000 */
         0x38, /* 0011100 */
         0x28, /* 0010100 */
         0x6C, /* 0110110 */
         0x7C, /* 0111110 */
         0xC6, /* 1100011 */
         0xC6, /* 1100011 */
         0x00, /* 0000000 */
     },
     /*
      * code=66, hex=0x42, ascii="B"
      */
     {
         0x00, /* 0000000 */
         0x7C, /* 0111110 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x7C, /* 0111110 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x7C, /* 0111110 */
         0x00, /* 0000000 */
     },
     /*
      * code=67, hex=0x43, ascii="C"
      */
     {
         0x00, /* 0000000 */
         0x3C, /* 0011110 */
         0x66, /* 0110011 */
         0x60, /* 0110000 */
         0x60, /* 0110000 */
         0x60, /* 0110000 */
         0x66, /* 0110011 */
         0x3C, /* 0011110 */
         0x00, /* 0000000 */
     },
     /*
      * code=68, hex=0x44, ascii="D"
      */
     {
         0x00, /* 0000000 */
         0x7C, /* 0111110 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x7C, /* 0111110 */
         0x00, /* 0000000 */
     },
     /*
      * code=69, hex=0x45, ascii="E"
      */
     {
         0x00, /* 0000000 */
         0x7E, /* 0111111 */
         0x66, /* 0110011 */
         0x60, /* 0110000 */
         0x78, /* 0111100 */
         0x60, /* 0110000 */
         0x66, /* 0110011 */
         0x7E, /* 0111111 */
         0x00, /* 0000000 */
     },
     /*
      * code=70, hex=0x46, ascii="F"
      */
     {
         0x00, /* 0000000 */
         0x7E, /* 0111111 */
         0x66, /* 0110011 */
         0x60, /* 0110000 */
         0x7C, /* 0111110 */
         0x60, /* 0110000 */
         0x60, /* 0110000 */
         0x60, /* 0110000 */
         0x00, /* 0000000 */
     },
     /*
      * code=71, hex=0x47, ascii="G"
      */
     {
         0x00, /* 0000000 */
         0x3C, /* 0011110 */
         0x66, /* 0110011 */
         0x60, /* 0110000 */
         0x6E, /* 0110111 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x3E, /* 0011111 */
         0x00, /* 0000000 */
     },
     /*
      * code=72, hex=0x48, ascii="H"
      */
     {
         0x00, /* 0000000 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x7E, /* 0111111 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x00, /* 0000000 */
     },
     /*
      * code=73, hex=0x49, ascii="I"
      */
     {
         0x00, /* 0000000 */
         0x3C, /* 0011110 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x3C, /* 0011110 */
         0x00, /* 0000000 */
     },
     /*
      * code=74, hex=0x4A, ascii="J"
      */
     {
         0x00, /* 0000000 */
         0x1E, /* 0001111 */
         0x0C, /* 0000110 */
         0x0C, /* 0000110 */
         0x0C, /* 0000110 */
         0x6C, /* 0110110 */
         0x6C, /* 0110110 */
         0x38, /* 0011100 */
         0x00, /* 0000000 */
     },
     /*
      * code=75, hex=0x4B, ascii="K"
      */
     {
         0x00, /* 0000000 */
         0x66, /* 0110011 */
         0x6C, /* 0110110 */
         0x6C, /* 0110110 */
         0x78, /* 0111100 */
         0x6C, /* 0110110 */
         0x6C, /* 0110110 */
         0x66, /* 0110011 */
         0x00, /* 0000000 */
     },
     /*
      * code=76, hex=0x4C, ascii="L"
      */
     {
         0x00, /* 0000000 */
         0x60, /* 0110000 */
         0x60, /* 0110000 */
         0x60, /* 0110000 */
         0x60, /* 0110000 */
         0x60, /* 0110000 */
         0x66, /* 0110011 */
         0x7E, /* 0111111 */
         0x00, /* 0000000 */
     },
     /*
      * code=77, hex=0x4D, ascii="M"
      */
     {
         0x00, /* 0000000 */
         0xC6, /* 1100011 */
         0xC6, /* 1100011 */
         0xEE, /* 1110111 */
         0xFE, /* 1111111 */
         0xD6, /* 1101011 */
         0xD6, /* 1101011 */
         0xD6, /* 1101011 */
         0x00, /* 0000000 */
     },
     /*
      * code=78, hex=0x4E, ascii="N"
      */
     {
         0x00, /* 0000000 */
         0x66, /* 0110011 */
         0x76, /* 0111011 */
         0x76, /* 0111011 */
         0x7E, /* 0111111 */
         0x6E, /* 0110111 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x00, /* 0000000 */
     },
     /*
      * code=79, hex=0x4F, ascii="O"
      */
     {
         0x00, /* 0000000 */
         0x3C, /* 0011110 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x3C, /* 0011110 */
         0x00, /* 0000000 */
     },
     /*
      * code=80, hex=0x50, ascii="P"
      */
     {
         0x00, /* 0000000 */
         0x7C, /* 0111110 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x7C, /* 0111110 */
         0x60, /* 0110000 */
         0x60, /* 0110000 */
         0x00, /* 0000000 */
     },
     /*
      * code=81, hex=0x51, ascii="Q"
      */
     {
         0x00, /* 0000000 */
         0x3C, /* 0011110 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x76, /* 0111011 */
         0x6E, /* 0110111 */
         0x3C, /* 0011110 */
         0x06, /* 0000011 */
     },
     /*
      * code=82, hex=0x52, ascii="R"
      */
     {
         0x00, /* 0000000 */
         0x7C, /* 0111110 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x6C, /* 0110110 */
         0x78, /* 0111100 */
         0x6C, /* 0110110 */
         0x66, /* 0110011 */
         0x00, /* 0000000 */
     },
     /*
      * code=83, hex=0x53, ascii="S"
      */
     {
         0x00, /* 0000000 */
         0x3C, /* 0011110 */
         0x66, /* 0110011 */
         0x60, /* 0110000 */
         0x3C, /* 0011110 */
         0x06, /* 0000011 */
         0x66, /* 0110011 */
         0x3C, /* 0011110 */
         0x00, /* 0000000 */
     },
     /*
      * code=84, hex=0x54, ascii="T"
      */
     {
         0x00, /* 0000000 */
         0x7E, /* 0111111 */
         0x5A, /* 0101101 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x3C, /* 0011110 */
         0x00, /* 0000000 */
     },
     /*
      * code=85, hex=0x55, ascii="U"
      */
     {
         0x00, /* 0000000 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x3C, /* 0011110 */
         0x00, /* 0000000 */
     },
     /*
      * code=86, hex=0x56, ascii="V"
      */
     {
         0x00, /* 0000000 */
         0xC6, /* 1100011 */
         0xC6, /* 1100011 */
         0x6C, /* 0110110 */
         0x6C, /* 0110110 */
         0x38, /* 0011100 */
         0x38, /* 0011100 */
         0x10, /* 0001000 */
         0x00, /* 0000000 */
     },
     /*
      * code=87, hex=0x57, ascii="W"
      */
     {
         0x00, /* 0000000 */
         0xC6, /* 1100011 */
         0xD6, /* 1101011 */
         0xD6, /* 1101011 */
         0xD6, /* 1101011 */
         0x7C, /* 0111110 */
         0x6C, /* 0110110 */
         0x44, /* 0100010 */
         0x00, /* 0000000 */
     },
     /*
      * code=88, hex=0x58, ascii="X"
      */
     {
         0x00, /* 0000000 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x3C, /* 0011110 */
         0x18, /* 0001100 */
         0x3C, /* 0011110 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x00, /* 0000000 */
     },
     /*
      * code=89, hex=0x59, ascii="Y"
      */
     {
         0x00, /* 0000000 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x3C, /* 0011110 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x3C, /* 0011110 */
         0x00, /* 0000000 */
     },
     /*
      * code=90, hex=0x5A, ascii="Z"
      */
     {
         0x00, /* 0000000 */
         0x7E, /* 0111111 */
         0x66, /* 0110011 */
         0x0C, /* 0000110 */
         0x18, /* 0001100 */
         0x30, /* 0011000 */
         0x66, /* 0110011 */
         0x7E, /* 0111111 */
         0x00, /* 0000000 */
     },
     /*
      * code=91, hex=0x5B, ascii="["
      */
     {
         0x00, /* 0000000 */
         0x3C, /* 0011110 */
         0x30, /* 0011000 */
         0x30, /* 0011000 */
         0x30, /* 0011000 */
         0x30, /* 0011000 */
         0x30, /* 0011000 */
         0x3C, /* 0011110 */
         0x00, /* 0000000 */
     },
     /*
      * code=92, hex=0x5C, ascii="\"
      */
     {
         0x00, /* 0000000 */
         0x60, /* 0110000 */
         0x60, /* 0110000 */
         0x30, /* 0011000 */
         0x30, /* 0011000 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x0C, /* 0000110 */
         0x00, /* 0000000 */
     },
     /*
      * code=93, hex=0x5D, ascii="]"
      */
     {
         0x00, /* 0000000 */
         0x3C, /* 0011110 */
         0x0C, /* 0000110 */
         0x0C, /* 0000110 */
         0x0C, /* 0000110 */
         0x0C, /* 0000110 */
         0x0C, /* 0000110 */
         0x3C, /* 0011110 */
         0x00, /* 0000000 */
     },
     /*
      * code=94, hex=0x5E, ascii="^"
      */
     {
         0x00, /* 0000000 */
         0x10, /* 0001000 */
         0x38, /* 0011100 */
         0x6C, /* 0110110 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
     },
     /*
      * code=95, hex=0x5F, ascii="_"
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x7C, /* 0111110 */
         0x7C, /* 0111110 */
     },
     /*
      * code=96, hex=0x60, ascii="`"
      */
     {
         0x00, /* 0000000 */
         0x30, /* 0011000 */
         0x30, /* 0011000 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
     },
     /*
      * code=97, hex=0x61, ascii="a"
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x3C, /* 0011110 */
         0x06, /* 0000011 */
         0x3E, /* 0011111 */
         0x66, /* 0110011 */
         0x3A, /* 0011101 */
         0x00, /* 0000000 */
     },
     /*
      * code=98, hex=0x62, ascii="b"
      */
     {
         0x00, /* 0000000 */
         0x60, /* 0110000 */
         0x60, /* 0110000 */
         0x7C, /* 0111110 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x5C, /* 0101110 */
         0x00, /* 0000000 */
     },
     /*
      * code=99, hex=0x63, ascii="c"
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x3C, /* 0011110 */
         0x66, /* 0110011 */
         0x60, /* 0110000 */
         0x66, /* 0110011 */
         0x3C, /* 0011110 */
         0x00, /* 0000000 */
     },
     /*
      * code=100, hex=0x64, ascii="d"
      */
     {
         0x00, /* 0000000 */
         0x06, /* 0000011 */
         0x06, /* 0000011 */
         0x3E, /* 0011111 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x3A, /* 0011101 */
         0x00, /* 0000000 */
     },
     /*
      * code=101, hex=0x65, ascii="e"
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x3C, /* 0011110 */
         0x66, /* 0110011 */
         0x7E, /* 0111111 */
         0x60, /* 0110000 */
         0x3C, /* 0011110 */
         0x00, /* 0000000 */
     },
     /*
      * code=102, hex=0x66, ascii="f"
      */
     {
         0x00, /* 0000000 */
         0x1C, /* 0001110 */
         0x36, /* 0011011 */
         0x30, /* 0011000 */
         0x7C, /* 0111110 */
         0x30, /* 0011000 */
         0x30, /* 0011000 */
         0x78, /* 0111100 */
         0x00, /* 0000000 */
     },
     /*
      * code=103, hex=0x67, ascii="g"
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x3A, /* 0011101 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x3E, /* 0011111 */
         0x06, /* 0000011 */
         0x3C, /* 0011110 */
     },
     /*
      * code=104, hex=0x68, ascii="h"
      */
     {
         0x00, /* 0000000 */
         0x60, /* 0110000 */
         0x60, /* 0110000 */
         0x6C, /* 0110110 */
         0x76, /* 0111011 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x00, /* 0000000 */
     },
     /*
      * code=105, hex=0x69, ascii="i"
      */
     {
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x00, /* 0000000 */
         0x18, /* 0001100 */
         0x38, /* 0011100 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x3C, /* 0011110 */
         0x00, /* 0000000 */
     },
     /*
      * code=106, hex=0x6A, ascii="j"
      */
     {
         0x0C, /* 0000110 */
         0x0C, /* 0000110 */
         0x00, /* 0000000 */
         0x0C, /* 0000110 */
         0x3C, /* 0011110 */
         0x0C, /* 0000110 */
         0x4C, /* 0100110 */
         0x6C, /* 0110110 */
         0x38, /* 0011100 */
     },
     /*
      * code=107, hex=0x6B, ascii="k"
      */
     {
         0x00, /* 0000000 */
         0x60, /* 0110000 */
         0x60, /* 0110000 */
         0x66, /* 0110011 */
         0x6C, /* 0110110 */
         0x78, /* 0111100 */
         0x6C, /* 0110110 */
         0x66, /* 0110011 */
         0x00, /* 0000000 */
     },
     /*
      * code=108, hex=0x6C, ascii="l"
      */
     {
         0x00, /* 0000000 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x3C, /* 0011110 */
         0x00, /* 0000000 */
     },
     /*
      * code=109, hex=0x6D, ascii="m"
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x6C, /* 0110110 */
         0xFE, /* 1111111 */
         0xD6, /* 1101011 */
         0xD6, /* 1101011 */
         0xD6, /* 1101011 */
         0x00, /* 0000000 */
     },
     /*
      * code=110, hex=0x6E, ascii="n"
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x6C, /* 0110110 */
         0x76, /* 0111011 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x00, /* 0000000 */
     },
     /*
      * code=111, hex=0x6F, ascii="o"
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x3C, /* 0011110 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x3C, /* 0011110 */
         0x00, /* 0000000 */
     },
     /*
      * code=112, hex=0x70, ascii="p"
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x7C, /* 0111110 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x76, /* 0111011 */
         0x6C, /* 0110110 */
         0x60, /* 0110000 */
     },
     /*
      * code=113, hex=0x71, ascii="q"
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x3A, /* 0011101 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x6E, /* 0110111 */
         0x36, /* 0011011 */
         0x06, /* 0000011 */
     },
     /*
      * code=114, hex=0x72, ascii="r"
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x26, /* 0010011 */
         0x7E, /* 0111111 */
         0x30, /* 0011000 */
         0x30, /* 0011000 */
         0x78, /* 0111100 */
         0x00, /* 0000000 */
     },
     /*
      * code=115, hex=0x73, ascii="s"
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x3C, /* 0011110 */
         0x60, /* 0110000 */
         0x3C, /* 0011110 */
         0x06, /* 0000011 */
         0x3C, /* 0011110 */
         0x00, /* 0000000 */
     },
     /*
      * code=116, hex=0x74, ascii="t"
      */
     {
         0x00, /* 0000000 */
         0x10, /* 0001000 */
         0x30, /* 0011000 */
         0xFC, /* 1111110 */
         0x30, /* 0011000 */
         0x30, /* 0011000 */
         0x36, /* 0011011 */
         0x1C, /* 0001110 */
         0x00, /* 0000000 */
     },
     /*
      * code=117, hex=0x75, ascii="u"
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x6E, /* 0110111 */
         0x36, /* 0011011 */
         0x00, /* 0000000 */
     },
     /*
      * code=118, hex=0x76, ascii="v"
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0xC6, /* 1100011 */
         0xC6, /* 1100011 */
         0x6C, /* 0110110 */
         0x38, /* 0011100 */
         0x10, /* 0001000 */
         0x00, /* 0000000 */
     },
     /*
      * code=119, hex=0x77, ascii="w"
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0xD6, /* 1101011 */
         0xD6, /* 1101011 */
         0x7C, /* 0111110 */
         0x6C, /* 0110110 */
         0x44, /* 0100010 */
         0x00, /* 0000000 */
     },
     /*
      * code=120, hex=0x78, ascii="x"
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0xC6, /* 1100011 */
         0x6C, /* 0110110 */
         0x38, /* 0011100 */
         0x6C, /* 0110110 */
         0xC6, /* 1100011 */
         0x00, /* 0000000 */
     },
     /*
      * code=121, hex=0x79, ascii="y"
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x66, /* 0110011 */
         0x66, /* 0110011 */
         0x36, /* 0011011 */
         0x1C, /* 0001110 */
         0x6C, /* 0110110 */
         0x38, /* 0011100 */
     },
     /*
      * code=122, hex=0x7A, ascii="z"
      */
     {
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x7E, /* 0111111 */
         0x06, /* 0000011 */
         0x18, /* 0001100 */
         0x30, /* 0011000 */
         0x7E, /* 0111111 */
         0x00, /* 0000000 */
     },
     /*
      * code=123, hex=0x7B, ascii="{"
      */
     {
         0x00, /* 0000000 */
         0x1C, /* 0001110 */
         0x30, /* 0011000 */
         0x30, /* 0011000 */
         0x60, /* 0110000 */
         0x30, /* 0011000 */
         0x30, /* 0011000 */
         0x1C, /* 0001110 */
         0x00, /* 0000000 */
     },
     /*
      * code=124, hex=0x7C, ascii="|"
      */
     {
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x00, /* 0000000 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x00, /* 0000000 */
     },
     /*
      * code=125, hex=0x7D, ascii="}"
      */
     {
         0x00, /* 0000000 */
         0x70, /* 0111000 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x0C, /* 0000110 */
         0x18, /* 0001100 */
         0x18, /* 0001100 */
         0x70, /* 0111000 */
         0x00, /* 0000000 */
     },
     /*
      * code=126, hex=0x7E, ascii="~"
      */
     {
         0x00, /* 0000000 */
         0x10, /* 0001000 */
         0x3A, /* 0011101 */
         0x6E, /* 0110111 */
         0x04, /* 0000010 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
         0x00, /* 0000000 */
     }}};

} // namespace Font
